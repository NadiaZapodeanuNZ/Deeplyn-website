...
const selectStyles = 
{
    control: (base, state) => ({...base, borderRadius: "0.5rem", borderColor: state.isFocused ? "#a855f7" : "#d1d5db",
        boxShadow: state.isFocused ? "0 0 0 2px rgba(168,85,247,0.35)" : "none",
        "&:hover": { borderColor: "#a855f7" }, fontSize: "0.875rem", minHeight: "2.5rem",backgroundColor: "#ffffff",}),

    placeholder: (base) => ({ ...base, color: "#9ca3af" }),
    option: (base, state) => ({...base, backgroundColor: state.isSelected ? "#a855f7" : state.isFocused ? "#f3e8ff" : "#ffffff",
        color: state.isSelected ? "#ffffff" : "#111827",fontSize: "0.875rem",cursor: "pointer",}),
...
{  
    ...
    const handleSubmit = async (e) => 
        ...
        e.preventDefault();
        setError("");
        const allFieldsFilled = form.first_name && form.last_name && 
                                form.username && form.email && form.password && 
                                form.confirm_pass && form.country;
    ...
    return (
        <div className="bg-white w-full max-w-sm md:max-w-md lg:max-w-lg rounded-2xl shadow-lg p-6 sm:p-8 lg:p-10">
            ...
            <form className="space-y-3" onSubmit={handleSubmit}>
                ...
                <div className="relative">
                    <input name="confirm_pass" type={showConfirmPass ? "text" : "password"} value={form.confirm_pass} onChange={handleChange}
            ...
            <p className="mt-5 text-center text-sm text-gray-500">
                Are you a therapist?{" "}
                <Link to="/register/therapist" className="text-purple-600 hover:underline font-medium">
