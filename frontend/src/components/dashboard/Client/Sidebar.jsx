...
import { LayoutDashboard, Users, CalendarDays, NotebookPen,MessageCircle, TrendingUp, Settings, HelpCircle, ChevronFirst, X, User, Brain , Dumbbell,TriangleAlert , LogOut} from "lucide-react";
...
function navLinkClasses(isActive) 
{
  ...
  if (isActive) 
    return base + " bg-white/10 text-white font-semibold";
...
function NavItem({ to, Icon, label, open, onClose }) 
{
  return (
    <NavLink to={to} onClick={onClose} className={({ isActive }) => navLinkClasses(isActive)}>
      {({ isActive }) => (<> <Icon size={20} className={iconColor(isActive)} />
  ...
  return (
    <aside className={`flex flex-col h-screen flex-shrink-0 transition-all duration-300 bg-gradient-to-b from-[#7B2FF7] via-[#5A4FF5] to-[#3B82F6] ${sidebarWidth(open)}`}>
      <div className="flex items-center gap-3 px-4 py-5">
        <div onClick={expandSidebar} className={logoContainerClasses(open)}
          role={!open ? "button" : undefined} aria-label={!open ? "Expand sidebar" : undefined}>
          <img src={logo} alt="Logo" className="w-5 h-5" />
        </div>
        {open && (<span className="text-white font-semibold text-base whitespace-nowrap">Deeplyn</span>)}
        {open && (<button onClick={collapseSidebar}
        ...
        <button onClick={onClose}
                ...
                aria-label="Close menu"><X size={18} />
        </button>
      </div>

      <nav className="flex-1 px-3 flex flex-col gap-2 mt-1">
        <NavItem to="/dashboard" Icon={LayoutDashboard} label="Dashboard" open={open} onClose={onClose} />
        <NavItem to="/client/daily-quiz" Icon={Brain} label="Daily Quiz" open={open} onClose={onClose} />
        <NavItem to="/client/journal" Icon={NotebookPen} label="Journal" open={open} onClose={onClose} />
        <NavItem to="/client/exercises" Icon={Dumbbell} label="Exercises" open={open} onClose={onClose} />
        <NavItem to="/client/therapist" Icon={User} label="Therapist" open={open} onClose={onClose} />
        <NavItem to="/client/profile" Icon={Settings} label="Settings" open={open} onClose={onClose} />
        {/* <NavItem to="/client/crisis" Icon={TriangleAlert} label="Crisis" open={open} onClose={onClose} /> */}
      </nav>
